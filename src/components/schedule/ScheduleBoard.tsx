...
export const ScheduleBoard: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs overflow-hidden flex flex-col">
      {/* 1. 컴팩트 상단 헤더 (제목 + 대상 학생 수 + 학년/학교/이름 정렬 버튼 + 충돌 버튼 + 학교 범례) */}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {conflicts.length > 0 ? (
            <button
              ...
              className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 text-xs font-bold cursor-pointer transition shadow-2xs animate-pulse"
              title="시간 충돌 세부 내역 보기"
            >
              <AlertTriangle className="w-3.5 h-3.5 text-red-600" />
              <span>충돌 {conflicts.length}건</span>
              <span className="text-[10px] bg-red-200 text-red-800 px-1 py-0.2 rounded font-bold">확인</span>
            </button>
          ) : (
      ...
      <div className="relative overflow-x-auto">
        <div className="min-w-[850px] flex flex-col">
          ...
          <div className="flex items-center border-b border-slate-200 bg-slate-50 sticky top-0 z-30 shadow-2xs">
            {/* 좌측 고정 열 헤더 (이름 / 학교 / 학년 열 제목 및 정렬 버튼 정렬) */}
            <div className="w-[230px] px-2.5 py-1.5 border-r border-slate-200 sticky top-0 left-0 z-40 bg-slate-100 text-xs font-bold text-slate-700 flex items-center shrink-0 shadow-2xs">
              ...
              <button
                ...
                className={`flex-1 min-w-0 flex items-center justify-start gap-1 px-1 py-0.5 rounded text-xs transition cursor-pointer ${
                  sortBy === 'name'
              ...
              >
                <span>이름</span>
                <ArrowUpDown className={`w-3 h-3 ${sortBy === 'name' ? 'text-white' : 'text-slate-400'}`} />
              </button>
              ...
              <button
                ...
                className={`w-14 shrink-0 flex items-center justify-center gap-0.5 px-1 py-0.5 rounded text-xs transition cursor-pointer ${
                  sortBy === 'school'
                    ? 'bg-blue-600 text-white font-black shadow-xs'
                    : 'text-slate-700 hover:bg-slate-200 font-bold'
                }`}
              ...
              >
                <span>학교</span>
                <ArrowUpDown className={`w-3 h-3 ${sortBy === 'school' ? 'text-white' : 'text-slate-400'}`} />
              </button>
              ...
              <button
                ...
                className={`w-10 shrink-0 flex items-center justify-center gap-0.5 px-1 py-0.5 rounded text-xs transition cursor-pointer ${
                  sortBy === 'grade'
                    ? 'bg-blue-600 text-white font-black shadow-xs'
                    : 'text-slate-700 hover:bg-slate-200 font-bold'
                }`}
              ...
              >
                <span>학년</span>
                <ArrowUpDown className={`w-3 h-3 ${sortBy === 'grade' ? 'text-white' : 'text-slate-400'}`} />
              </button>
            </div>
            {/* 우측 시간축 */}
